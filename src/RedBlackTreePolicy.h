...
*
* RedBlackTreePolicy.h
*
* A Red-Black Tree implementation using generic programming.
*/

#ifndef GRAPHKIT_SRC_RED_BLACK_TREE_POLICY_H
#define GRAPHKIT_SRC_RED_BLACK_TREE_POLICY_H

#include <cassert>
#include <functional>
#include "RedBlackNodePolicy.h"
...
namespace gk {
	...
	>
	class RedBlackTreePolicy {
	protected:
		typedef gk::RedBlackNodePolicy<T, U, K, O> RedBlackNode;
		RedBlackNode* nil_;
		RedBlackNode* root_;
		O size_;

		inline void leftRotate(RedBlackNode* x) noexcept {
			auto y = x->right_;
		...
		inline void rightRotate(RedBlackNode* y) noexcept {
			auto x = y->left_;
		...
		inline void transplant(RedBlackNode* u, RedBlackNode* v) noexcept {
			if (u->parent_ == nil_) {
				root_ = v;
		...
		inline RedBlackNode* minimum(RedBlackNode* x) const noexcept {
			auto y = nil_;
			while (x != nil_) {
				y = x;
				x = x->left_;
		...
		inline RedBlackNode* maximum(RedBlackNode* x) const noexcept {
			auto y = nil_;
			while (x != nil_) {
				y = x;
				x = x->right_;
		...
		inline void insertCleanUp(RedBlackNode* z) noexcept {
		...
		inline void removeCleanUp(RedBlackNode* x) noexcept {
		...
		inline RedBlackNode* internalFindByKey(const K& key) const noexcept {
			auto z = root_;
			while (z != nil_) {
				if (key == z->key_) {
					return z;
		...
		inline RedBlackNode* internalSelect(RedBlackNode *x, const O& order) const noexcept {
		...
		inline O& internalOrder(RedBlackNode* x) const noexcept {
			auto r = x->left_->order_ + 1;
			auto y = x;
			while (y != root_) {
		...
		inline RedBlackNode* internalInsert(const K& key, T* data) noexcept {
			auto y = nil_;
			auto x = root_;
			...
			auto z = new RedBlackNode{y, nil_, key, data};
		...
		inline RedBlackNode* internalRemove(const K& key) noexcept {
	...
	public:
		...
		using Node = RedBlackNode;
		using Type = T;
		using Key = K;
		using Order = O;

		// defaults
		RedBlackTreePolicy(const RedBlackTreePolicy&) = default;
		RedBlackTreePolicy& operator= (const RedBlackTreePolicy&) = default;
		RedBlackTreePolicy(RedBlackTreePolicy&&) = default;
		RedBlackTreePolicy& operator= (RedBlackTreePolicy&&) = default;
		...
		inline const RedBlackNode* node(const O& order) const noexcept {
	...
	protected:
		RedBlackTreePolicy() noexcept
			: nil_{new RedBlackNode{}}, root_{nil_}, size_{} {
		}

		virtual ~RedBlackTreePolicy() {
			clear();
			delete nil_;
		}
	};
