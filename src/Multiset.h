...
namespace gk {
	...
	public:
		explicit Multiset(G* graph) noexcept;
		virtual ~Multiset();
		...
		static gk::Multiset<G, T, O>* Instance(v8::Isolate* isolate, G* graph) noexcept;
		static GK_INIT(Init);
...
GK_CONSTRUCTOR(gk::Multiset<G, T, O>::constructor_);

template <typename G, typename T, typename O>
gk::Multiset<G, T, O>::Multiset(G* graph) noexcept
	: gk::Export{},
...
template <typename G, typename T, typename O>
gk::Multiset<G, T, O>::~Multiset() {
	cleanUp();
	graph_->Unref();
}

template <typename G, typename T, typename O>
bool gk::Multiset<G, T, O>::insert(v8::Isolate* isolate, T* node) noexcept {
	if (!node->indexed()) {
...
template <typename G, typename T, typename O>
bool gk::Multiset<G, T, O>::remove(T* node) noexcept {
	if (!node->indexed()) {
		return false;
	}
	return gk::RedBlackTree<T, false, std::string, O>::remove(node->hash(), [&](T* n) {
		n->Unref();
	});
}

template <typename G, typename T, typename O>
bool gk::Multiset<G, T, O>::remove(const std::string& k) noexcept {
...
template <typename G, typename T, typename O>
void gk::Multiset<G, T, O>::cleanUp() noexcept {
	this->clear([&](T *n) {
		n->Unref();
	});
}
...
template <typename G, typename T, typename O>
gk::Multiset<G, T, O>* gk::Multiset<G, T, O>::Instance(v8::Isolate* isolate, G* graph) noexcept {
	const int argc = 1;
	v8::Local<v8::Value> argv[argc] = {graph->handle()};
	auto cons = GK_FUNCTION(constructor_);
	return node::ObjectWrap::Unwrap<gk::Multiset<G, T, O>>(cons->NewInstance(argc, argv));
}

template <typename G, typename T, typename O>
GK_INIT(gk::Multiset<G, T, O>::Init) {
	GK_SCOPE();
...
template <typename G, typename T, typename O>
GK_METHOD(gk::Multiset<G, T, O>::New) {
	GK_SCOPE();
	...
	if (args.IsConstructCall()) {
		...
		auto obj = new gk::Multiset<G, T, O>{g};
		obj->Wrap(args.This());
		GK_RETURN(args.This());
	} else {
		const int argc = 1;
...
template <typename G, typename T, typename O>
GK_METHOD(gk::Multiset<G, T, O>::Size) {
	GK_SCOPE();
	auto s = node::ObjectWrap::Unwrap<gk::Multiset<G, T, O>>(args.Holder());
	GK_RETURN(GK_NUMBER(s->size()));
}

template <typename G, typename T, typename O>
GK_METHOD(gk::Multiset<G, T, O>::Insert) {
	GK_SCOPE();
...
template <typename G, typename T, typename O>
GK_METHOD(gk::Multiset<G, T, O>::Remove) {
	GK_SCOPE();
...
template <typename G, typename T, typename O>
GK_METHOD(gk::Multiset<G, T, O>::Clear) {
	GK_SCOPE();
	auto s = node::ObjectWrap::Unwrap<gk::Multiset<G, T, O>>(args.Holder());
	s->cleanUp();
...
template <typename G, typename T, typename O>
GK_METHOD(gk::Multiset<G, T, O>::Find) {
	GK_SCOPE();
...
template <typename G, typename T, typename O>
GK_INDEX_GETTER(gk::Multiset<G, T, O>::IndexGetter) {
	GK_SCOPE();
...
template <typename G, typename T, typename O>
GK_PROPERTY_GETTER(gk::Multiset<G, T, O>::PropertyGetter) {
	GK_SCOPE();
}
...
template <typename G, typename T, typename O>
GK_PROPERTY_ENUMERATOR(gk::Multiset<G, T, O>::PropertyEnumerator) {
	GK_SCOPE();
	auto i = node::ObjectWrap::Unwrap<gk::Multiset<G, T, O>>(args.Holder());
	auto is = i->size();
